...
public class ListingService {
    ...
    public Map<String, Object> generateSkuPlans(Map<String, Object> req) throws Exception {
        ...
        String prompt = String.format(
            ...
            + "【硬约束·严格遵守】components 里的 itemCode **只能**取自上面【共享配件清单】和【批量件清单】，"
            + "**绝对禁止**放入任何整支花洒/喷头主体/整机（即【主件清单】里的编码或名称含\"手喷/单花洒/花洒\"的整机）。"
            + "配件只能是软管、底座/支架、滤芯这类附件。把整机当配件拼进型号会导致成本和价格严重错误。\n\n"
            + "【数量档位规则（重点）】\n"
            ...
            + "- **【硬性要求】型号名必须逐一列清该型号实际包含的每个配件，禁止用「全配」「全套」「套装」这种笼统词概括**；"
            + "例如含 底座+软管+5滤芯 的型号要写成「喷头+稳固银底座+1.5米防爆软管+5支滤芯」，绝不能写「全配+5支滤芯」。\n"
